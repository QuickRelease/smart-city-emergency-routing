...
if not mapName:
    mapName = input(
        "Please enter map name, available maps are: %s: "
        % ", ".join(SCENARIO_LOCATION_CONFIG.keys())
    )
if not scenarioNum:
    scenarioNum = int(
        input(
            "Please enter scenario number, available numbers are: %s: "
            % ", ".join(str(n) for n in SCENARIO_NUMBER_CONFIGS.keys())
        )
    )
