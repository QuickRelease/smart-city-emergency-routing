...
class IntersectionController:
    ...
    def calculateNewReservedTime(self, pv, reservedTime):
        ...
        return (
            (0.5 if self.zip else 1.5)
            + reservedTime
            + (lenThruJunc / (pv.getSpeed() or 1))
        )
    ...
    def _eligibleZippings(self, platoon):
        if len(self.platoonZips) >= 1:
            ...
            if (
                platoon.getLanePositionFromFront() - z[-1].getLanePositionFromFront()
                < 10
                and platoon.getLanePositionFromFront()
                - z[-1].getLanePositionFromFront()
                > 0
            ):
                return z
    ...
    def removeIrreleventPlatoons(self):
        ...
        for p in self.platoons:
            if not p.isActive() or all(
                [l not in self.lanesServed for l in p.getLanesOfAllVehicles()]
            ):
        ...
        if self.platoons:
            ...
            if self.zip:
                for v in self.getVehicleZipOrderThroughJunc():
                    if v.isActive():
                        setSpeed = (
                            v._previouslySetValues["setSpeed"]
                            if "setSpeed" in v._previouslySetValues
                            else "None"
                        )
                        logging.info(
                            "Vehicle: %s, Target: %s, Current: %s",
                            v.getName(),
                            setSpeed,
                            v.getSpeed(),
                        )
